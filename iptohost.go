...
func main() {
	...
	if *outputFileName != "" {
		outputFile, outputFileErr = os.OpenFile(*outputFileName, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0644)
	...
	seenHosts := make(map[string]bool) // Track seen hostnames
	...
	for {
		...
		case res := <-resChan:
			...
			fmt.Println(res)
			if outputFile != nil && !seenHosts[res] {
